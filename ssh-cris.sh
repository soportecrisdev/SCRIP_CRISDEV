...
draw_header() {
    ...
    echo -e "${CYAN}╠════════════════════════════════════════════════════════════════╣${NC}"
    printf "${CYAN}║${WHITE}  IP Pública:   ${GREEN}%-18s${WHITE} SO:     ${YELLOW}%-22s${CYAN}║\n" "$ip" "${os:0:22}"
    printf "${CYAN}║${WHITE}  Memoria RAM:  ${GREEN}%-18s${WHITE} CPU:    ${YELLOW}%-22s${CYAN}║\n" "${ram_used}MB / ${ram_total}MB (${ram_pct}%)" "$cpu_load | $uptime_str"
    printf "${CYAN}║${WHITE}  SSH Online:   ${GREEN}%-18s${WHITE} DB Reg: ${YELLOW}%-22s${CYAN}║\n" "${users_online} conectado(s)" "${users_total} usuario(s)"
    echo -e "${CYAN}╠════════════════════════════════════════════════════════════════╣${NC}"
    echo -e "${CYAN}║${WHITE}                     ESTADO DE PROTOCOLOS                       ${CYAN}║${NC}"
    echo -e "${CYAN}╠════════════════════════════════════════════════════════════════╣${NC}"
    printf "${CYAN}║${WHITE} • BHTTP Relay:  %-54b${CYAN}║\n" "$(get_bhttp_live_status)"
    printf "${CYAN}║${WHITE} • UDP CRIS:     %-54b${CYAN}║\n" "$(get_udpcris_live_status)"
    printf "${CYAN}║${WHITE} • OpenSSH:      %-54b${CYAN}║\n" "$(get_ssh_live_status)"
    printf "${CYAN}║${WHITE} • Stunnel SSL:  %-54b${CYAN}║\n" "$(get_ssl_live_status)"
    printf "${CYAN}║${WHITE} • Dropbear:     %-54b${CYAN}║\n" "$(get_dropbear_live_status)"
    printf "${CYAN}║${WHITE} • SlowDNS:      %-54b${CYAN}║\n" "$(get_slowdns_live_status)"
    echo -e "${CYAN}╚════════════════════════════════════════════════════════════════╝${NC}"
    echo ""
}
